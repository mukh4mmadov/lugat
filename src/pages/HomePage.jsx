    ...
    <div className="space-y-8">
      <section className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
        <GlassCard className="relative overflow-hidden p-8 md:p-10">
          ...
          <WordBadge>Official source only</WordBadge>
          <h2 className="mt-5 max-w-3xl text-4xl font-black tracking-tight md:text-6xl">
            Premium K-TALIM 1A Korean vocabulary practice.
          </h2>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-300">
            Study the exact vocabulary extracted from the uploaded K-TALIM 1A book images: flashcards, listening, quizzes, writing, favorites, and smart review.
          </p>
        ...
        <GlassCard className="grid content-between gap-5">
          <div>
            <WordBadge tone="green">Progress</WordBadge>
            <h3 className="mt-4 text-3xl font-black">Your study dashboard</h3>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Metric label="Words" value={totalWords} />
            <Metric label="Lessons" value="14" />
            <Metric label="Completed" value={completedLessons} />
            <Metric label="Streak" value={progress.stats.streak} />
          </div>
        </GlassCard>
      </section>
      ...
      <GlassCard className="grid gap-5 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <WordBadge tone="amber">Daily Challenge</WordBadge>
          <h2 className="mt-3 text-3xl font-black">Today's focused session: Lesson {dailyLesson}</h2>
          <p className="mt-2 text-slate-600 dark:text-slate-300">Warm up with flashcards, then finish with quiz and writing practice.</p>
        </div>
